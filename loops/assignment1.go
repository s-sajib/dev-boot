...
It should return the total cost (as a float64) to send a batch of numMessages messages. Each message
costs 1.0, plus an additional fee. The fee structure is:
...
func bulkSend(numMessages int) float64 {
	...
	for i := 0; i < numMessages; i++ {
		additionalCost += float64(i) * 0.01
	}
	return float64(numMessages) + additionalCost
}
